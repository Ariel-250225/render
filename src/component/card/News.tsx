      ...
      <Card color={image}>
        <CardShape width={width}>
          {banner ? (
            ...
            <Image src={image} aspectRatio={aspectRatio}></Image>
          )}
          <ShapeDividerBottom>
            {shape && <Layer color={color} />}
          </ShapeDividerBottom>
        </CardShape>
        <CardBody>
...
const Image = styled.img<{ aspectRatio?: string }>(
  ({ aspectRatio = "1 / 1" }) => css`
    width: 100%;
...
function CardFooter(props: {
  ...
  return (
    <CardFooterContainer theme={theme} color={color}>
      {description && (
        <Description color={color} onClick={() => window.open(description)}>
          {description}
        </Description>
      )}
      <small>{subLine}</small>
